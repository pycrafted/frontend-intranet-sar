...
export function Navbar({ onMenuClick }: NavbarProps) {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-navbar-border bg-navbar text-navbar-foreground shadow-enterprise">
      <div className="flex h-16 items-center justify-between px-2 sm:px-4 lg:px-6 navbar-mobile-optimized">
        {/* Left section */}
        <div className="flex items-center gap-2 sm:gap-4 min-w-0 flex-shrink-0">
          <Button
            variant="ghost"
            size="sm"
            className="lg:hidden text-navbar-foreground hover:bg-navbar-foreground/10 flex-shrink-0"
            onClick={onMenuClick}
          >
        ...
        <div className="flex items-center gap-1 sm:gap-2 flex-shrink-0">
          ...
          {/* Notifications */}
          <Button variant="ghost" size="sm" className="relative text-navbar-foreground hover:bg-navbar-foreground/10 flex-shrink-0">
            <Bell className="h-4 w-4 sm:h-5 sm:w-5 navbar-icon" />
            <Badge className="absolute -top-1 -right-1 h-4 w-4 sm:h-5 sm:w-5 rounded-full p-0 text-xs bg-destructive text-destructive-foreground flex items-center justify-center">
              3
            </Badge>
          </Button>
          ...
          {isAuthenticated && user ? (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="ghost"
                  className="flex items-center gap-1 sm:gap-2 text-white hover:bg-[#2a323d] flex-shrink-0 min-w-0"
                  style={{ backgroundColor: '#353E4B' }}
                >
                  <Avatar className="h-6 w-6 sm:h-8 sm:w-8 flex-shrink-0 navbar-avatar">
              ...
              <DropdownMenuContent align="end" className="w-56">
                <DropdownMenuLabel>
                  <div className="flex flex-col space-y-1">
                    <p className="text-sm font-medium">{authUtils.getFullName(user)}</p>
