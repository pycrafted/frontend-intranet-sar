        ...
        <div className="flex-1 overflow-y-auto">
          {isLoading ? (
            ...
            <>
              ...
              {/* Séparateur */}
              {forums.length > 0 && conversations.length > 0 && !isCollapsed && (
                <div className="px-4 py-2 border-t border-slate-200/20">
                  <h4 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Conversations</h4>
                </div>
              )}
              ...
              const conversationIdStr = typeof conversation.id === 'number' ? conversation.id.toString() : conversation.id
              const isActive = selectedConversationId === conversationIdStr || selectedConversationId === conversation.id?.toString()
              return (
                <button
                  key={conversation.id}
                ...
                >
                  ...
                  {!isCollapsed && (
                    <>
                      ...
                      <div className="flex-1 min-w-0 relative z-10">
                        <div className="flex items-start justify-between gap-2 mb-1">
                          <h4 className="font-medium text-sm line-clamp-2 flex-1">{conversation.title}</h4>
                          {isActive && (
                            <div className="w-2 h-2 rounded-full bg-blue-400 animate-pulse flex-shrink-0 mt-1" />
                          )}
                        </div>
                        <p className="text-xs text-slate-400 line-clamp-2 mb-2">{conversation.description}</p>
                        <div className="flex items-center gap-3">
                          <Badge 
                            variant="secondary" 
                            className={cn(
