...
import type { RefItem, AdvanceFormData } from '@/lib/types/sirh'
import { X, Loader2 } from 'lucide-react'

interface AdvanceModalProps {
  advanceTypes: RefItem[]
  onClose: () => void
  onSubmit: (data: AdvanceFormData) => Promise<number>
}

export function AdvanceModal({ advanceTypes, onClose, onSubmit }: AdvanceModalProps) {
  const [form, setForm] = useState<Partial<AdvanceFormData>>({})
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!form.advance_type_id || !form.amount || !form.description) return
    setSubmitting(true)
    setError(null)
    try {
      await onSubmit(form as AdvanceFormData)
      setSuccess(true)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <h3 className="text-lg font-semibold text-gray-800">Nouvelle avance sur salaire</h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={e => setForm(f => ({ ...f, advance_type_id: Number(e.target.value) }))}
            >
