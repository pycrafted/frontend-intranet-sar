...
import type { RefItem, EditingFormData } from '@/lib/types/sirh'
import { X, Loader2 } from 'lucide-react'

interface EditingModalProps {
  editingTypes: RefItem[]
  onClose: () => void
  onSubmit: (data: EditingFormData) => Promise<number>
}

export function EditingModal({ editingTypes, onClose, onSubmit }: EditingModalProps) {
  const [form, setForm] = useState<Partial<EditingFormData>>({})
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!form.editing_type_id || !form.reason) return
    setSubmitting(true)
    setError(null)
    try {
      await onSubmit(form as EditingFormData)
      setSuccess(true)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <h3 className="text-lg font-semibold text-gray-800">Modification de fiche</h3>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Type de modification *</label>
            <select
              required
              className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#344256]"
              value={form.editing_type_id ?? ''}
              onChange={e => setForm(f => ({ ...f, editing_type_id: Number(e.target.value) }))}
            >
