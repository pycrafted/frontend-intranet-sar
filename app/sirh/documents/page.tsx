...
import { DocumentList } from '@/components/sirh/documents/DocumentList'
import { DocumentModal } from '@/components/sirh/documents/DocumentModal'
      ...
      <LayoutWrapper>
        <div className="w-full max-w-4xl mx-auto space-y-6 overflow-hidden">
          ...
          {sirh.loading ? (
            ...
            <DocumentList documentRequests={sirh.documentRequests} documentTypes={sirh.documentTypes} onNew={() => setModal(true)} />
          )}

          {modal && (
            <DocumentModal documentTypes={sirh.documentTypes} onClose={() => setModal(false)} onSubmit={sirh.createDocumentRequest} />
          )}
        </div>
      </LayoutWrapper>
    </AuthGuard>
